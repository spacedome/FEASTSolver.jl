...
- Sparse and distributed moment-NLFEAST are prototyped only inside the
  ...
  turns that split into actions: increase contour resolution for visible packet
  ...
  is a fixed-target negative control: accepted changed-radius charts that
  change the packet count are classified as `chart_changes_packet`, not
  accepted for the original target.
...
- Fixed-target chart steering:
  ...
  different spectral packet. This is a guardrail rather than an automatic
  radius-change policy.
...
| Defective matrix-valued multiplicity | `torture_matrix.jl` row `:defective_triangular_multiplicity` | A triangular repeated-sine/Jordan-like NEP closes count `15` as five multiplicity-three packets, exercising coalesced matrix eigenvectors rather than scalar repeated roots only. | Covered |
| Clustered simple roots below local resolution | `torture_matrix.jl` row `:clustered_simple_roots` | Twelve simple roots arranged as six close pairs are accepted as six count-certified multiplicity-two packets. This is a packet-level success but a geometric-resolution diagnostic boundary. | Diagnostic boundary |
| Polynomial lower-rung consistency | `torture_matrix.jl` row `:near_multiple_polynomial_companion` | Polynomial-native extraction matches companion FEAST on clustered polynomial roots: companion and all native stages match `18/18`. | Covered |
| Operators with branch cuts | `torture_matrix.jl` rows `:branch_cut_fixed_sheet`, `:near_branch_fixed_sheet`, and `:branch_cut_operator` | The fixed-sheet principal square-root case is executable and count-complete, including a near-branch conditioning stress with branch points just outside the contour; cross-cut/multisheet cases remain `:documented_gap` until branch/sheet/analytic-domain data are part of the model. | Diagnostic boundary plus documented gap |
...
| Executable adversarial torture profile | `test/torture/moment_rii.jl` tests `generated adversarial NEP profiles` and `expanded adversarial NEP profiles` | Runs the fixed-sheet branch control, meromorphic pole-ladder boundary, multiplicity-four control, defective triangular multiplicity, clustered simple-root packets, near-branch fixed-sheet conditioning, and polynomial companion/native agreement through `run_moment_rii_torture_case`. | Added as `:moment_heavy` torture |
