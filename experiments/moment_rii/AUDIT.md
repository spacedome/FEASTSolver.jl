...
- Sparse and distributed moment-NLFEAST are not implemented. The experiment now
  ...
  updates. It deliberately does not yet provide symbolic sparse reuse,
  distributed sparse factor storage, sparse work buffers, or performance
  claims. It also has a partition diagnostic proving that
  ...
  through the same persistent worker boundary, but this is still not distributed
