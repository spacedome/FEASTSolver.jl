...
- Sparse and distributed moment-NLFEAST are not implemented. The experiment now
  ...
  diagnostic sends residual bases to actual Julia worker processes and reduces
  the returned Laurent blocks to match the serial update, but it is not yet a
  persistent worker plan with cached operator data, reusable sparse
  factorizations, or benchmark-level performance claims.
