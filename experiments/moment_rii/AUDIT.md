...
- More problem classes are still needed before calling the algorithm
  ...
  disagreement plus local count warnings in a non-triangular no-oracle run, and
  the triangular multiplicity test covers the algebraic-count completion branch.
...
1. Push the non-triangular mixed-policy stress across at least one additional
   fully coupled problem class or a larger/different contour to check that the
   weak-center refinement is not tuned only to the two-delay toy.
