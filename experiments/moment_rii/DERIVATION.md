...
2. **Scalar RII compatibility.** If the chart has a diagonal scalar state and
   ...
   `r_j` is generated by the same Laurent moments because

   ```text
   1/(z - lambda_j) = (1/r) * sum_{ell >= 0} alpha_j^ell zeta^(-ell-1).
   ```

   A finite `K_update` is therefore a truncated local RII correction in the
   chart coordinate. In the linear case the FEAST/RII algebraic identity below
   collapses this back to the usual rational filter exactly.
...
- **Local RII truncation:** for simple roots whose chart coordinates satisfy
  `|alpha_j| < rho < 1`, the finite Laurent update is the degree-`K_update`
  polynomial approximation to the scalar residual-inverse correction, with an
  error bounded by the omitted Laurent tail and the conditioning of
  `T(z)^(-1)` on the contour.
