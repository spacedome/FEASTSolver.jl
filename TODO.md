...
- [ ] Implement SS, Sakurai subspace iteration algorithms
  ...
  - Chart-policy checkpoint: the stiff four-function analytic control exposed that operator-relative residuals can hide displaced values when one analytic component dominates `norm(T(lambda))`. Strict vector residuals plus contour-local component scaling make the local charts honest.
  - Local-cover result: a residual-scored grid cover of scaled local charts recovers all 44 unique roots of the radius-20 `sin/cos/sin(z)-0.3/exp(z)-1` control. This points to scaled local chart covers and merge policies as the robust generalized moment-NLFEAST direction, rather than one huge monomial chart or ad-hoc scalar deflation.
  - Nonnormal analytic control: an upper-triangular analytic operator with the same known determinant roots exposed a chart-cover grid bug and a radius-selection issue. Centering the grid at the contour center and trying multiple local radii recovers all 44 unique roots even with triangular coupling 10.
  - Multiplicity control: `sin(z)^2` on radius 10 recovers algebraic multiplicity 14, and a triangular shared-root `sin/exp(z)-1` chart recovers both algebraic copies at zero. Keep this as diagnostic evidence unless Jordan-chain information improves ordinary subspace quality; explicit Jordan handling should not become a core requirement by default.
  - Escalation-ladder result: on the scaled local chart cover, pure counted-SS with zero residual updates recovers all 44 roots for the simultaneously similar analytic control. On the triangular nonnormal control with coupling 10, zero/one/two residual Laurent updates recover 34/42/44 roots. This supports an escalation ladder: cheap local SS/Beyn extraction first, residual FEAST updates only when diagnostics show the local realization is not enough.
